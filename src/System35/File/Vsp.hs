module System35.File.Vsp (
  module System35.File.Vsp.Base,
  module System35.File.Vsp.Pixel,
  convert, decode, tokenize, getImage,
) where
...
tokenize d
    ...
    where
      (block, skip) = getToken d
-- (data, opcode) -> (block, rest)
getToken :: S.ByteString -> (ByteBlock, Int)
getToken d
    | opcode == 0x00 = (ByteBlock { code = 0x00, bytes = S.take 2 d, len = getL d }, 2) -- 0x00, len
    | opcode == 0x01 = (ByteBlock { code = 0x01, bytes = S.take 3 d, len = getL d }, 3)
    | opcode == 0x02 = (ByteBlock { code = 0x02, bytes = S.take 4 d, len = 2 * getL d }, 4)
    | opcode == 0x03 = (ByteBlock { code = 0x03, bytes = S.take 2 d, len = getL d}, 2)
    | opcode == 0x04 = (ByteBlock { code = 0x04, bytes = S.take 2 d, len = getL d}, 2)
    | opcode == 0x05 = (ByteBlock { code = 0x05, bytes = S.take 2 d, len = getL d}, 2)
    | opcode == 0x06 = (ByteBlock { code = 0x06, bytes = S.take 1 d, len = 0}, 1)
    | opcode == 0x07 = (ByteBlock { code = 0x07, bytes = S.take 2 d, len = 1}, 2)
    | otherwise      = (ByteBlock { code = opcode, bytes = S.empty,  len = 1}, 1)
    where
      opcode :: Word8
      opcode = S.head d
